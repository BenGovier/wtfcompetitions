-- Migration: allow 'dg_football' as a fourth campaigns.reveal_type value.
--
...
--
-- Existing 'normal', 'scratch_card' and 'treasure_chest' campaigns are
-- unaffected. Mirrors 006-add-treasure-chest-reveal-type.sql exactly.
...
-- 2) Recreate it allowing exactly the four supported values.
ALTER TABLE public.campaigns
  ADD CONSTRAINT campaigns_reveal_type_check
  CHECK (reveal_type IN ('normal', 'scratch_card', 'treasure_chest', 'dg_football'));
