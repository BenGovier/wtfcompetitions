...
import { findIdentityFields, type GrowthDashboardPayload } from './growth'

export type FetchGrowthResult =
  | { ok: true; data: GrowthDashboardPayload; filters: ReportFilters }
  | { ok: false; error: string; status: number }

/** RPC argument object for supabase.rpc('get_admin_growth_dashboard', ...). */
function toGrowthRpcArgs(filters: ReportFilters) {
  return {
...
/**
 * Fetch the Growth dashboard payload from the single
 ...
 * fail loudly if this is ever wired up ahead of an auth guard. Growth reuses the
 * exact same filter parsing / date semantics as Overview.
 */
export async function fetchGrowthDashboard(
  ...
  if (leaks.length > 0) {
    console.log('[v0] growth payload identity guard tripped:', leaks.join(','))
    return { ok: false, error: 'query_failed', status: 500 }
  }
