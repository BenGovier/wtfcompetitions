...
export function TicketSelector({ basePrice, bundles: rawBundles, campaignId, soldCount, capTotal, startsAt, endsAt, ticketsSold, hardCapTotalTickets, isFreeEntry, freeEntryLimitPerUser }: TicketSelectorProps) {
  ...
  const handleEnter = async () => {
    if (!hasAcceptedTerms) {
      setError("Please confirm you are 18+ and agree to the T&Cs to continue.")
      return
    }
      ...
      <div className="space-y-3 rounded-2xl border border-white/10 bg-white/5 p-5 shadow-2xl backdrop-blur-lg">
        ...
        <label className="flex items-start gap-3 cursor-pointer select-none">
          <input
            ...
            className="mt-0.5 h-5 w-5 shrink-0 rounded border-purple-500/50 bg-white/10 text-amber-500 focus:ring-2 focus:ring-amber-400 focus:ring-offset-0 accent-amber-500"
          />
