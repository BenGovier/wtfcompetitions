...
// Guard: fail loudly if the route ever imports the delivery worker or a mail
// provider. These modules must NEVER be pulled in by the materialisation cron.
...
const URL_STR = 'https://app.example/api/cron/marketing-materialisation'
...
describe('Marketing materialisation cron route (GET /api/cron/marketing-materialisation)', () => {
  ...
  it('valid Bearer secret invokes the materialisation RPC exactly once', async () => {
  ...
  it('invokes exactly materialize_marketing_recipients with p_limit = 100', async () => {
    await GET(makeRequest({ authorization: 'Bearer top-secret' }))
    expect(rpc).toHaveBeenCalledTimes(1)
    expect(rpc).toHaveBeenCalledWith('materialize_marketing_recipients', { p_limit: 100 })
  })
  ...
  it('never invokes the discovery RPC or any other RPC', async () => {
    await GET(makeRequest({ authorization: 'Bearer top-secret' }))
    const rpcNames = rpc.mock.calls.map((c) => c[0])
    expect(rpcNames).toEqual(['materialize_marketing_recipients'])
    expect(rpcNames).not.toContain('discover_marketing_opportunities')
  })
  ...
  it('fails closed with a stable code on RPC error (no raw message leaked)', async () => {
    ...
    const json = await res.json()
    expect(json).toEqual({ ok: false, error: 'materialisation_failed' })
...
describe('vercel.json cron configuration', () => {
  ...
  it('contains the dedicated materialisation cron at */10 * * * *', () => {
    expect(byPath.get('/api/cron/marketing-materialisation')).toBe('*/10 * * * *')
  })

  it('there is exactly ONE materialisation cron entry', () => {
    const entries = cfg.crons.filter((c) => c.path === '/api/cron/marketing-materialisation')
  ...
  it('leaves the discovery cron untouched', () => {
