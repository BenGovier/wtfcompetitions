...
// Guard: the preparation route must NEVER pull in the delivery worker.
...
const URL_STR = 'https://app.example/api/cron/marketing-preparation'
...
describe('Marketing preparation cron route (GET /api/cron/marketing-preparation)', () => {
  ...
  it('valid Bearer secret invokes the preparation RPC exactly once', async () => {
  ...
  it('invokes exactly prepare_marketing_recipient_content with p_limit = 100', async () => {
    await GET(makeRequest({ authorization: 'Bearer top-secret' }))
    expect(rpc).toHaveBeenCalledWith('prepare_marketing_recipient_content', { p_limit: 100 })
  })
  ...
  it('never invokes delivery, readiness, materialisation, or any other RPC', async () => {
    await GET(makeRequest({ authorization: 'Bearer top-secret' }))
  ...
  it('fails closed with a stable code on RPC error (no raw message leaked)', async () => {
    ...
    const json = await res.json()
    expect(json).toEqual({ ok: false, error: 'preparation_failed' })
...
describe('vercel.json preparation cron configuration', () => {
